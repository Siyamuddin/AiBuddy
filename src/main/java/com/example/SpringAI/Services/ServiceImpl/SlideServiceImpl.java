...
public class SlideServiceImpl implements SlideServices {
    ...
    public String generateShortQuestions(Long slideId, String numberOfQuestions) {
        Slide slide=slideRepo.findById(slideId).orElseThrow(()-> new ResourceNotFoundException("Slide","slide ID: ",slideId));
        String questions = rag.generateRAGResponse(slide.getSlideContent()," You are an AI assistant who helps student prepare for exam using their lecture slide, please Generate "+ numberOfQuestions+ " short questions and answers from this lecture that may asked int exam");
    ...
    public String generateMCQ(Long slideId, String numberOfMCQs) {
        Slide slide=slideRepo.findById(slideId).orElseThrow(()-> new ResourceNotFoundException("Slide","slide ID: ",slideId));
        String MCQ=rag.generateRAGResponse(slide.getSlideContent()," You are an AI assistant who helps student prepare for exam using their lecture slide, please Generate "+numberOfMCQs+" important multiple-choice questions (MCQs) based on the content. Ensure that each question has 4 options, and highlight the correct answer.");
    ...
    public String generateSummary(Long slideId) {
        ...
        String summary = rag.generateRAGResponse(slide.getSlideContent(),"Summarize the key points from the content above for effective revision, don't miss any important point.");
    ...
    public String writeAiQuery(Long slideId, String query) {
        Slide slide=slideRepo.findById(slideId).orElseThrow(()-> new ResourceNotFoundException("Slide","slide ID: ",slideId));
        String AiResponse=rag.generateRAGResponse(slide.getSlideContent(),"  You are an AI assistant who helps student prepare for exam using their lecture slide, please answer the question "+query);
        return AiResponse;
    }
